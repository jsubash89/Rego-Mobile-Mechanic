...
## Summary
Customer books a service from vehicle/service selection through confirmation and status tracking.
...
## Entry points
vehicle, service, fulfillment, provider/slot, estimate, confirmation
...
## Business rules
Booking cannot confirm until required inputs are valid; downstream incompatible choices are reset when upstream choices change.
